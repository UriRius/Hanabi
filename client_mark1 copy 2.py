...
#class extensions
class CartaJuego(game.Card):
    def __init__(self, name, id, value, color, tag) -> None:
        super().__init__(self,id,value,color)
        self.name = name 
        self.tag = tag
        
    def EsJugable(self):
        #This function returns True if playing the card is save
        carta = "Card "+ self.value + " - " + self.color
        #the card has been played
        if carta in PlayedCards:
            return False
        #it's a 1 and there are no cards in table
        if self.value == "1" and carta not in PlayedCards:
            return True
...
def iniciaPistes():
    #final format -> [[[Player, "", ""],["0", "None", "Res"],..., ..., ..., ...], [....]]
    Pistes = []
    i = 0
    for p in  Players:
        Pistes[i][0] = p.name
        for j in range(5):
            Pistes[i][j] = "0 None Res"
        i += 1
    return Pistes

def addHints(NewHint):
    #<pos> <valor/color> <player>
    TODO

def addHintsForIA(NewHint):
    
    cardposition = NewHint.split(" ")[0]
    cardhint = NewHint.split(" ")[1] #could be a number or a color
    
    nums = ["1", "2", "3", "4", "5"]

    if cardhint in nums:
        Hand[int(cardposition)][0] = cardhint 
        
    else:
        Hand[int(cardposition)][1] = cardhint 
    ...
    while run:
        ...
        if type(data) is GameData.ServerGameStateData:
            ...
            for p in data.players:
                ...
                print("AIXO ES UNA CARTA")
            
                for card in p.hand:
                    pp = CartaJuego(p.name,card.id,card.value,card.color, "")

                    print("Carta: "+ str(card.value) + " - " + str(card.color))

            print("Table cards: ")
            PlayedCards = data.tableCards
